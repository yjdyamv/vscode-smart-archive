...
// ════════════════════════════════════════════════════════════════════
// System 7-Zip detection and compression
// ════════════════════════════════════════════════════════════════════
...
describe("system 7-Zip", () => {
    ...
    for (const name of process.platform === "win32" ? ["7z.exe", "7za.exe"] : ["7z", "7za"]) {
      try {
        ...
        if (r.status === 0 && r.stdout.length > 0) {
          ...
          if (require("fs").existsSync(found)) return found;
        }
      } catch {
        continue;
      }
  ...
  itOrSkip("compresses and decompresses a directory", () => {
    const tmpDir = require("fs").mkdtempSync(
      require("path").join(require("os").tmpdir(), "sat_sz_"),
    );
    const srcDir = require("path").join(tmpDir, "src");
    require("fs").mkdirSync(srcDir, { recursive: true });
    require("fs").writeFileSync(require("path").join(srcDir, "hello.txt"), "hello world");
    require("fs").writeFileSync(require("path").join(tmpDir, "readme.md"), "# test");

    const archive = require("path").join(tmpDir, "test.7z");
    const outDir = require("path").join(tmpDir, "out");

    // Compress
    let r = spawnSync(sz!, ["a", "-t7z", "-mx5", archive, srcDir, require("path").join(tmpDir, "readme.md")], { stdio: "pipe", timeout: 30000 });
    expect(r.status).toBe(0);

    // Verify archive exists
    expect(require("fs").existsSync(archive)).toBe(true);
    expect(require("fs").statSync(archive).size).toBeGreaterThan(0);

    // List
    r = spawnSync(sz!, ["l", "-slt", archive], { stdio: "pipe", timeout: 10000 });
    const listing = r.stdout.toString();
    expect(listing).toContain("hello.txt");
    expect(listing).toContain("readme.md");

    // Decompress
    r = spawnSync(sz!, ["x", `-o${outDir}`, archive], { stdio: "pipe", timeout: 30000 });
    expect(r.status).toBe(0);

    const outSrc = require("path").join(outDir, "src", "hello.txt");
    const outReadme = require("path").join(outDir, "readme.md");
    expect(require("fs").existsSync(outSrc)).toBe(true);
    expect(require("fs").existsSync(outReadme)).toBe(true);
    expect(require("fs").readFileSync(outSrc, "utf-8")).toBe("hello world");
...
// ════════════════════════════════════════════════════════════════════
// parse7zListing — archive self-reference filtering
// ════════════════════════════════════════════════════════════════════
...
describe("parse7zListing archive self-reference filter", () => {
  ...
  it("filters by base name when path case differs", () => {
    const results = [
  ...
  it("keeps inner file whose base name matches archive name", () => {
    ...
    // archive is "/archive.7z" (VFS), inner file is "other/dir/archive.7z" — NOT filtered
    const filtered = filterSelfRefs(results, "archive.7z", "/archive.7z");

    expect(filtered.length).toBe(2); // both survive — base name alone is not enough to filter
...
// ════════════════════════════════════════════════════════════════════
// Archive naming conventions (promptSavePath, uniquePath, getFullExt)
// ════════════════════════════════════════════════════════════════════

describe("archive naming conventions", () => {
  // getFullExt inline — can't import from constants (transitive vscode dep)
  const COMPOUND = [".tar.zst", ".tar.xz", ".tar.bz2", ".tar.gz", ".tgz", ".tbz2", ".tbz", ".txz", ".tzst", ".tar.lz", ".tar.lzma", ".tlz"];
  ...
  it("keep original name + append format (promptSavePath single-file)", () => {
    function defName(filePath: string, format: string): string {
      return path.posix.join(path.posix.dirname(filePath), path.posix.basename(filePath) + "." + format);
    }
