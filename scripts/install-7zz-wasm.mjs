 ...
 * vendor/7zz-wasm/package.json is generated here too — the whole vendor/
 * tree is git-ignored and rebuilt by the install scripts, so a fresh
 * checkout needs only `npm ci` (postinstall) to get a complete engine.
 *
...
async function main() {
  ...
  // The whole vendor/ tree is git-ignored and regenerated by this script, so
  // package.json is written on every run (idempotent) — the version can never
  // drift from the staged engine, and a fresh checkout gets a complete
  // vendor/7zz-wasm/ after postinstall.
  ...
  console.log(`[7zz-wasm] wrote vendor/7zz-wasm/package.json (version ${getWasmPackageVersion()})`);
